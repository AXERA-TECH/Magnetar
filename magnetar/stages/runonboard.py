...
def run(task_dir: Path, sample: np.ndarray, target_hw: str, pwd: str, cpp_binary: Path | None = None) -> dict | None:
    from magnetar.board_util import (
        BoardBusyError, acquire_board_lease, board_workdir, ensure_remote_infer,
        probe_board_env, release_board_lease, select_board,
    ...
    # 板端独占租约：被占用则本阶段跳过（板端验证是可选项，不阻塞交付）
    try:
        lease = acquire_board_lease(board, note=f"runonboard {task_dir.name}")
    except BoardBusyError as e:
        from magnetar.stages.state import mark_stage
        mark_stage(task_dir, "RUNONBOARD", status="skipped", summary=str(e))
        print(f"[RUNONBOARD] {e}")
        return None
    try:
        in_npy = rb / "input.npy"; in_bin = rb / "input.bin"
        np.save(in_npy, sample.astype(np.float32)); sample.astype(np.float32).tofile(in_bin)
        rd = board_workdir(lease, "work")
        ssh(board, f"mkdir -p {rd}")
