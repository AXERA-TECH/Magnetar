...
- `pulsar2_available`：本地 PULSAR2_BIN / PULSAR2_IMAGE / PATH / hf-mirror 镜像任一可用 → COMPILE；全不可用 → blocked
- `export_valid`：静态 shape + ORT 加载 + 对分通过 → TOOLCHAIN；失败 → 换导出策略重试或问用户
- `accuracy_gate`：cosine ≥ 0.99 → SDK-GEN；失败 → 查 issues/，无匹配 STOP 并提议 QAT（官方 QAT.axera）
...
- 需问用户：SOURCE/TARGET_HARDWARE 缺失、私有凭据、Pulsar2 不可用、仅有随机校准数据、
  精度不达标无已知修复、板端凭据、多次导出失败、发布目标/仓库/凭据
