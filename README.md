...
Magnetar 提供两种方式查看流水线状态：

### TUI 实时监控

```bash
# 监控最近一次任务
./bin/magnetar monitor

# 监控指定任务
./bin/magnetar monitor todos/work/20260720-mymodel/
```

终端内实时展示 9 阶段流水线进度、各阶段状态、关键指标（ONNX 大小、AXMODEL 大小、cosine、延迟等）。按 `Ctrl+C` 退出。
...
# 生成指定任务的报告
./bin/magnetar report todos/work/20260720-mymodel/
```

生成自包含 `dashboard.html`，可用浏览器直接打开。包含流水线进度条、关键指标卡片、阶段详情表。适合分享给团队或归档。
